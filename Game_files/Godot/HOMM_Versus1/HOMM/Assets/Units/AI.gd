...
func AI_Fightmode():
	...
	tempo = []
	tempo = retrieveOnRangeEnemies(onRange_Enemies, onRange_Enemy_counter, 
	...
	Tiles_scores = tempo[0]
	Tiles_scores = tempo[1]
...
func retrieveOnRangeEnemies(return_onRange_Enemies, return_onRange_Enemy_counter,
	...
	for i in internal_Enemy_counter:
		...
		|| abs(internal_Enemies[i].global_position.y) != DISPLACEMENT+1)):
			...
			return_onRange_Enemies[j] = TURN.get_child(i)
			j += 1
			
	return [return_onRange_Enemies, return_onRange_Enemy_counter]
...
func retrieveTiles(return_TileList, return_Tile_counter, return_Tiles_scores,
	...
	if internal_onRange_Enemy_counter > 0:
		...
		tempo = checkTilesAroundTarget(	return_TileList, return_Tile_counter, 
										0, -64)
...
func scoreTiles(return_Tiles_scores, return_Tile_counter, 
	...
	return [return_Tiles_scores, return_Tile_counter]
