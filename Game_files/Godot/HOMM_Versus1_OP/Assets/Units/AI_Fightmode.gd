...
func AI_Fightmode():
	...
	# 7. scoreTiles
	tempo = []
	tempo = scoreTiles(Tiles_scores, Tile_counter, TileList,
						Enemies, Enemy_counter)
	TileList = tempo[0]
	Tile_counter = tempo[1]
	Tiles_scores = tempo[2]
...
func scoreEnemies(return_Enemies_scores, internal_Enemies, internal_Enemy_counter):
	if internal_Enemy_counter > 0:
		...
		for i in internal_Enemy_counter:
			if (abs(internal_Enemies[i].global_position.x - self.global_position.x) <= 64*DISPLACEMENT
			&& abs(internal_Enemies[i].global_position.y - self.global_position.y) <= 64*DISPLACEMENT):
...
func scoreTiles(return_Tiles_scores, return_Tile_counter, return_TileList,
				internal_Enemies, internal_Enemy_counter):
	...
	# Si ennemis à portée, on cherche la tuile d'attaque à portée.
		for i in return_Tile_counter:
			...
			for j in internal_Enemy_counter:
				...
				if (abs(return_TileList[i].x - internal_Enemies[j].global_position.x) < Enemy_Displacement
				&& (abs(return_TileList[i].y - internal_Enemies[j].global_position.y) < Enemy_Displacement
				&&(abs(internal_Enemies[j].global_position.x) != Enemy_Displacement
				|| abs(internal_Enemies[j].global_position.y) != Enemy_Displacement))):
	...
	# Sinon, on cherche la tuile la plus intéressante pour le prochain tour.
		for i in TEMPORARY.get_child_count():
			...
			var Tile_is_OK : bool = true
			for j in Character_number:
				if ((abs(CurrentTilePosition.x - TURN.get_child(j).global_position.x) > 63
				&& abs(CurrentTilePosition.y - TURN.get_child(j).global_position.y) > 63)
				||CurrentTilePosition.x > 0 || CurrentTilePosition.x < get_viewport().size.x
				||CurrentTilePosition.y > 0 || CurrentTilePosition.y < get_viewport().size.y):
					pass
				else:
					Tile_is_OK = false
					break
			...
			if Tile_is_OK == true:
				...
				return_Tile_counter +=1
				return_TileList.append(0)
				return_TileList[return_Tile_counter-1] = CurrentTilePosition
				# On cherche les tuiles qui permettent d'attaquer au tour suivant 
				#(+1 par ennemi accessible)
				for j in internal_Enemy_counter:
					if (abs(CurrentTilePosition.x - internal_Enemies[j].global_position.x) < 64*DISPLACEMENT
					&& abs(CurrentTilePosition.y - internal_Enemies[j].global_position.y) < 64*DISPLACEMENT):
						return_Tiles_scores[return_Tile_counter-1] += 1
	
	if return_TileList[0].x < 0 || return_TileList[0].y < 0:
		pass
	return [return_TileList, return_Tile_counter, return_Tiles_scores]
...
func chooseTile(return_Tile_target, 
	...
	if internal_Tile_counter > 1:
		...
		if return_Tile_target.x < 0 || return_Tile_target.y < 0:
			pass
	...
	# en l'absence de cible convaincante, on vise la tuile la plus proche du centre.
		...
		for i in TEMPORARY.get_child_count():
			...
			if abs(TEMPORARY.get_child(i).global_position.x - BattlefieldCenter.x) < PreviousTarget.x:
				PreviousTarget = TEMPORARY.get_child(i).global_position
				...
				if abs(TEMPORARY.get_child(i).position.y - BattlefieldCenter.x) < PreviousTarget.y:
					PreviousTarget = TEMPORARY.get_child(i).global_position.y
					return_Tile_target = TEMPORARY.get_child(i).global_position
#		print("Battle center : ", BattlefieldCenter)
	
	print(STATS.NAME, " could have moved to ", return_Tile_target)
	return_Tile_target.x = min(max(return_Tile_target.x, 0), 1400)
	return_Tile_target.y = min(max(return_Tile_target.y, 0), 768)
	print(STATS.NAME, " should move to ", return_Tile_target)
