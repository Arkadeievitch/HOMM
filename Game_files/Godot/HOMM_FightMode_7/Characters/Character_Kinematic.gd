...
func ConnectSelf():
	...
	# warning-ignore:return_value_discarded
	TEMPORARY.connect("Tiles_deleted", self, "PlayAction")
...
func PlayAction(action, tile):
	...
	if(active_turn == true
		...
		&& 	displacement_allowed == true):
		displacement()
		# Signal emissions triggered by "tween_completed"
...
func increment_priorities():
	...
	# TURN : au bout N signaux, 
		...
		# Signal Priorities_retrieved pour générer les nouvelles tuiles.
...
func Character_attacked(Action_position, Tile_position):
	...
	var SIDE_ATT = ATTACKER.SIDE#
	var NAME_ATT = ATTACKER.NAME#
	var SIDE_DEf = STATS.SIDE   #
