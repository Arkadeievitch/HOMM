...
var COLOR_SCROLLBAR : Node

func _ready():
	if (get_parent().get_parent().get_parent().has_node("Unit_BG")):
		self.modulate = get_parent().get_parent().color
		
		if has_node("/root/MainNode/SelectionMenu"):
			var DEFAULT_ARMY = get_parent().get_parent().get_parent().get_parent()
			var TABLE = DEFAULT_ARMY.get_parent().get_parent().get_parent().get_parent()
			COLOR_SCROLLBAR = TABLE.get_node("Color_ScrollBar")
			
			# warning-ignore:return_value_discarded
			COLOR_SCROLLBAR.connect("ColorChanged", self,"changeColor")
